...
export const triggerWorkflow = async (
  ...
  if (state.triggers[webhookId]?.debug) {
    addDebugLog('Triggering workflow with payload:', {
      ...payload,
      ...{ url: `${baseUrl}/webhook${state.testMode ? '-test' : ''}/${webhookId}/webhook` },
    });
  }

  const res = await axios
    .post(`${baseUrl}/webhook${state.testMode ? '-test' : ''}/${webhookId}/webhook`, payload, {
      headers,
    })
    .catch((e) => {
      if (state.triggers[webhookId]?.debug) {
        addDebugLog('Error triggering workflow:', e);
      }
      console.log(e);
  ...
  if (res && state.triggers[webhookId]?.debug) {
    addDebugLog('Workflow triggered successfully:', res.data);
  }
